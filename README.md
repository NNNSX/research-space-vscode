...
`v2.1.0-alpha.41` 当前属于 **蓝图创建 / 加载 / 运行闭环收口预览**，已经可以安装和体验，但**稳定性仍弱于 `v1.2.2` 稳定版**。

- 适合：体验节点组、Pipeline、保存状态、加载提示与执行错误可见性等已收口的新链路，以及“选区工作流 → 蓝图草稿 → 保存蓝图文件 → 从蓝图库恢复结构化实例 → 实例级运行”的连续链路；当前蓝图实例化已经开始恢复原始结构，输入占位已支持外部节点正式绑定与拖近替换，实例外层也已从临时 Board 壳升级为正式蓝图容器，输出占位支持运行后自动回填，实例容器开始持久化最近一次运行摘要、最近成功/失败时间；取消运行时会显示“取消中 / 已取消”，最近问题节点也可以从容器中直接定位；旧功能节点现在会按最新工具定义自动补齐 schema / 默认参数，旧蓝图实例也开始按当前蓝图 definition 首轮补齐缺失的占位、内部节点和定义边；AIHubMix 自定义 provider 在遇到未识别的流式返回格式时，会优先保留原始 SSE 内容而不是静默写出空文件；聊天类功能节点现在统一支持“最大上下文 / 最大输出 tokens”设置：已知模型上限时自动取模型最大值，若用户在设置中填了更小上限，则按 `min(模型最大值, 设置值)` 执行；设置面板也开始支持“常用模型”管理，功能节点默认只展示常用模型，且常用模型支持排序；其中 AIHubMix 的 LLM 默认常用模型先收口为 `gemini-3.1-pro-preview` / `gpt-5.4` / `claude-sonnet-4-6`，多模态模型继续沿用各自设置中的默认值；画布在拿到设置快照后也会先后台预取已配置 provider 的模型列表，功能节点与设置面板共用去重后的模型请求入口，减少重开画布后多个节点重复触发模型拉取；同时默认常用模型提示会先按 provider 给出乐观默认值，不再那么容易在重开画布后从“无提示”再补跳成“仅显示常用模型”；常用模型过滤也不再因为空配置或未命中而悄悄回退成全量模型列表；功能节点外层和内容裁切层也已拆开，左右通道圆点不会再被裁掉半边；功能节点里的下拉已直接恢复为常规原生样式，不再继续使用自定义浮层，从而避开缩放变形、位置错位和边界联动问题；缺失输入提示也已去重，避免同一节点同时出现蓝色引导和黄色 warning 两条“需连接输入”；视角恢复链路也补做了一轮收口：`.rsws` 中已有的 `viewport` 会在画布数据就绪后再次显式应用，切标签或关闭前若还有待保存改动也会尝试立即冲刷，降低重开后又回到初始区域的问题；二级设置弹窗中的文本服务商设置也已进一步细分为概览 / 内置服务商 / 自定义服务商标签；重新打开画布时会优先恢复到上次关闭前的视角，而不再总是回到初始 fitView 位置；同时函数节点的自动高度同步也做了第二轮收口，减少运行按钮溢出或底部大块留白；顶栏保存按钮在“未保存 / 保存中 / 保存失败”三种状态下也会给出更明显的高亮或动效提醒，降低忘记保存的概率；设置页现在也会明确显示“设置已保存 / 待保存 / 保存中”，并额外提供一个手动“保存设置”按钮，减少用户对自动保存机制的不确定感；同时已修复 `alpha.40` 中点击设置直接触发 React error #310 的回归
- 不适合：唯一生产环境、不可回退的重要工作区
- 建议：先备份工作区，再在副本中试用；重要项目建议继续保留 `v1.2.2`
- 当前仍在继续：旧蓝图实例的“严格迁移”还没做完；目前更偏向补齐缺失结构，不会激进删除所有历史残留节点/边
...
**蓝图库与蓝图实例**：左侧 AI 工具面板现在会显示当前工作区的蓝图库。你可以把某个蓝图拖到画布上，或直接点击 `+` 把它实例化回画布。当前实例化已经开始按完整蓝图定义恢复功能节点、内部数据节点、输入占位节点、输出占位节点，并生成正式的蓝图实例容器节点；这个容器会接管标题、实例状态、输入校验和运行入口，而不再继续复用普通 Board 壳。输入占位现在既可以直接接受外部数据节点/节点组连线，也支持把真实数据节点拖近占位后高亮吸附、松手替换，占位原有内部边会自动重绑到真实节点；点击容器上的 `▶ 运行` 后，会先校验必填输入，再仅对该实例内部功能节点生成执行计划并按拓扑运行，中间产物与最终输出会优先就地更新/回填到实例结构中；容器中栏会继续显示当前步骤，而最近一次运行摘要、最近成功时间和最近失败时间也会持久化回画布，便于复盘。当前仍有边界：实例外功能节点不能作为该蓝图实例的 Pipeline 上游。
...
当前版本：**v2.1.0-alpha.41**（2026-04-18）
