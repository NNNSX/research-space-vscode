...
`2.1.1-alpha.1` 当前属于 **蓝图创建 / 加载 / 运行闭环收口预览**，已经可以安装和体验，但**稳定性仍弱于 `v1.2.2` 稳定版**。

- 适合：体验蓝图创建 / 加载 / 实例运行、节点组、Pipeline、多模态工具、模型选择与保存状态等当前已接通的新链路；当前旧蓝图实例、旧节点组、失效占位节点、悬空连线、历史 board 伪节点、旧 `summaryGroups` 以及部分旧壳数据（viewport / boards / stagingNodes）都开始做更严格的加载期清理，历史残留结构会逐步收口；蓝图主视图当前已进一步切到画板式 overlay 外壳：外框与标题栏独立渲染，内部原始 pipeline 节点继续作为唯一主要信息层，蓝图容器节点本身退出主视图命中链路；同时这条 overlay 订阅链已补稳定快照修复，避免重启后打开画布直接触发 React Render Error；实例容器不再被统一 size 归一逻辑拉回固定槽位尺寸，拖拽也改成 overlay 快照式整实例平移，继续收口边界迟到恢复、误拖与延迟；已保存蓝图现在也支持回编辑、另存为，以及从画布上蓝图实例直接回灌成新的蓝图草稿；槽位级错误提示也开始从隐藏容器下沉到真实节点层，输入/输出占位会直接显示缺失绑定、等待产出或失败未产出的状态，并能直接联动关联上游节点；输出槽位也开始正式尊重“允许多输出”配置：单输出槽位维持一个当前输出，多输出槽位则会保留多个当前结果；输出历史弹窗也开始明确区分“当前版 / 上一版 / 更早历史”，并补了快捷打开/恢复上一版；同时切画布或重开画布时，旧画布遗留的蓝图临时状态也会更彻底地清空重建，蓝图 `index / definitions` 的异步回填继续扩展为按画布 session 统一隔离，连 request index、蓝图草稿保存/删除后的索引刷新，以及树视图侧直接回推 init 也开始走同一口径；自动保存联合回归也继续推进，除树视图删除节点与 Markdown/JSON 导出优先读取 live canvas、file watcher 的 change/delete/create 开始按批量 pending URI 聚合处理、保存失败回执开始区分“当前失败”与“过期失败”外，这几轮全局审查蓝图引擎后，蓝图输入绑定统计、输出绑定计数、“最新输出”选择，以及旧蓝图实例里缺失的输出 placeholder 骨架恢复也开始逐步统一到当前语义；最近几轮继续把“蓝图最终输出掉线”往数据结构层收口：现在不仅宿主写盘会直接带上 blueprint output 绑定元数据与 bound-output → placeholder 绑定边，单输出槽位在运行态也不再删除输出 placeholder，而是保留稳定槽位骨架、只替换当前 bound output，并把旧节点上的外连线重绑到新当前输出；同时 blueprint 内部 direct pipeline 产出的 runtime 中间结果也开始按隐藏内部节点落盘与加载恢复，不再像普通输出那样弹到蓝图外；最近几轮继续补连接体验：蓝图输入占位会把“接受类型”前置成显眼卡片，类型不匹配、单绑定已满、命中外框而非槽位圆点等连接失败场景会直接弹出明确原因，而且这类提示不再误套“网络错误”；这一轮发布已从零散 alpha 小版本汇总为新的 `2.1.1-alpha.1`：蓝图主线重点收口了旧实例/旧画布严格迁移、overlay 外壳与整实例拖拽、输出槽位历史与失败后继续、蓝图最终输出持久化/恢复、隐式 output slot 自动补齐、输出节点外置后重开不再掉线，以及输入/输出占位样式与自动测高统一；同时 Copilot 默认模型也已收口到 `gpt-4.1`，蓝图创建弹窗文本选择误关与普通文件节点 footer 回归都已一并修复；失败后继续、实例运行历史、旧蓝图 definitions 恢复与局部损坏降级恢复能力仍在继续收口；多模态输入在发请求前也会先按图片数量 / 体积 / 估算开销做预算裁剪，减少大图或多图直冲 provider 时报错
- 不适合：唯一生产环境、不可回退的重要工作区
- 建议：先备份工作区，再在副本中试用；重要项目建议继续保留 `v1.2.2`
- 当前仍在继续：旧蓝图实例与旧画布元素的“严格迁移”还没做完；目前更偏向高置信度清理与补齐，不会激进删除所有历史残留节点/边
- 提取说明：新增文档格式的文本提取正在继续做跨平台收口；目前 macOS 支持最完整，OOXML / ODF / EPUB 已改为不依赖系统 `unzip`，但 `doc/xls/ppt` 一类 legacy 格式仍属于 best-effort
- 详细版本变化：请查看 `CHANGELOG.md` 或 GitHub Releases 页面；发布说明会按“上一个已发布版本 → 当前版本”汇总
...
当前版本：**v2.1.0-alpha.96**（2026-04-19）
