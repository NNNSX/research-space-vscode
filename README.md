...
- 当 AIHubMix 返回 `insufficient_user_quota` 时，节点内会提示“额度不足”
...
**右键菜单**：节点、节点组、画板和连接线都有上下文菜单。常用操作包括打开文件、重命名、复制、删除、编辑画板、删除连接线等。

**删除确认**：删除节点、连接线、节点组、画板、暂存项、任务项、对话记录、自定义服务商或常用模型时，会先弹出确认，降低误删风险。

**输入与状态提示**：AI 功能节点会在运行前提示缺失输入；输入框支持鼠标拖拽选中文本，不会被画布拖拽抢走。AI 输出节点会显示服务商与模型标签。

**任务清单**：任务节点支持 checklist 状态，完成项会在节点内显示勾选标记，并在 `.md` 文件中写回 `- [x]` / `- [ ]`。

**连线与通道**：节点遵循左入右出；普通数据流连线与 Pipeline 连线在选中后会高亮。
...
**文件爆炸（MinerU）**：在 **AI Tools → 通用 → 文件爆炸** 中添加工具节点，连接 1 个受支持文件后运行。当前支持 PDF、DOCX、PPT/PPTX、XLS/XLSX 和图片；PDF / Office 文档优先走 MinerU 在线解析，XLS/XLSX 可走本地结构化拆解。结果会生成文本节点、图片节点和文档关系索引，并作为节点组输出给后续 AI 工具使用。
...
**蓝图库与蓝图实例**：左侧 AI 工具面板会显示当前工作区的蓝图库。蓝图可拖到画布，或点击 `+` 实例化。实例会恢复功能节点、内部数据节点、输入占位、输出占位和蓝图容器；运行时会校验必填输入，按拓扑执行实例内部功能节点，并把中间产物与最终输出回填到实例结构中。右键蓝图实例标题栏，可将当前实例结构回灌为新的蓝图草稿。限制：实例外功能节点不能作为该蓝图实例的 Pipeline 上游。
