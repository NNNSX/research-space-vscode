...
**右键菜单**：右键任意节点（数据/功能）弹出上下文菜单，支持**打开文件**、**重命名**（笔记节点）、**复制节点**、**从画布删除**；AI 输出节点额外支持**查看生成历史**（可恢复历史版本）。

键盘快捷键：选中节点后按 `Delete` 或 `Backspace` 删除；`Ctrl+Z` / `Cmd+Z` 撤销；`Ctrl+Shift+Z` / `Cmd+Shift+Z` 重做。

**节点调整大小**：选中节点后拖拽边角可自由缩放，内容自动适应新尺寸。

**拖拽导入**：从 VSCode 资源管理器或系统文件管理器直接拖拽文件到画布（自动添加到暂存架）。按住 `Shift` 拖拽可直接放入画布。

**选区与归纳**：工具栏「⬚ 选区」按钮切换框选模式，选中多个节点后可创建归纳组（命名矩形框，支持编辑名称和颜色）。
