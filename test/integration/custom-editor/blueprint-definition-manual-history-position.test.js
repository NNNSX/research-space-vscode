...
async function buildTempCanvasUri() {
  ...
  const targetPath = path.join(tempDir, `blueprint-definition-manual-history-${Date.now()}.rsws`);
...
async function run() {
  ...
  try {
    ...
    assert.ok(container, 'Expected manual-history fixture canvas to contain a blueprint container.');
    assert.ok(boundOutput, 'Expected manual-history fixture canvas to contain a blueprint bound output.');
    ...
    rawCanvas.metadata.updated_at = '2026-04-21T18:42:00.000Z';
    ...
    const rebound = await waitFor(async () => {
      ...
      if (output1.meta?.blueprint_bound_slot_id !== 'output_slot_summary' || output2.meta?.blueprint_bound_slot_id !== 'output_slot_summary') {
        return null;
      }
      if (output1.position.x !== 1120 || output1.position.y !== 420) { return null; }
    ...
    assert.equal(reopenedOutputs.length, 2, 'Expected both manually moved history outputs to survive definition-driven rebind + reopen.');
    ...
    assert.deepEqual(reopenedOutput1.position, { x: 1120, y: 420 });
    assert.deepEqual(reopenedOutput2.position, { x: 1280, y: 520 });
