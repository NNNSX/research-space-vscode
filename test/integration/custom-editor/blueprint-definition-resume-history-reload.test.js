...
async function buildTempCanvasUri() {
  ...
  const targetPath = path.join(tempDir, `blueprint-definition-resume-history-${Date.now()}.rsws`);
...
async function writeBlueprintDefinition(filePath) {
  const definition = {
    ...
    metadata: {
      created_at: '2026-04-21T23:50:00.000Z',
      source_canvas_title: '蓝图失败恢复输出稳定性回归',
    },
  };
...
async function run() {
  ...
  try {
    ...
    assert.ok(container, 'Expected resume-history fixture canvas to contain a blueprint container.');
    assert.ok(boundOutput, 'Expected resume-history fixture canvas to contain a blueprint bound output.');
    ...
    rawCanvas.nodes.push({
      ...
      file_path: 'outputs/summary_resume_manual_2.md',
      meta: {
        ai_provider: 'copilot',
        ai_model: 'gpt-4.1',
    ...
    rawCanvas.metadata.updated_at = '2026-04-21T23:52:00.000Z';
    ...
    const resumedCanvas = await readCanvas(canvasUri);
    const resumedContainer = resumedCanvas.nodes.find(node => node.id === 'bp-container');
    assert.ok(resumedContainer);
    resumedContainer.meta = {
      ...(resumedContainer.meta ?? {}),
      blueprint_last_run_status: 'succeeded',
      blueprint_last_issue_node_id: undefined,
      blueprint_last_issue_node_title: undefined,
    };
    resumedCanvas.nodes.push({
      id: 'bound-output-3',
      node_type: 'ai_output',
      title: '摘要结果（继续执行后）',
      position: { x: 0, y: 0 },
      size: { width: 240, height: 160 },
      file_path: 'outputs/summary_resume_3.md',
      meta: {
        ai_provider: 'copilot',
        ai_model: 'gpt-4.1',
        blueprint_def_id: 'bp-def-stable-output',
    ...
    resumedCanvas.metadata.updated_at = '2026-04-21T23:54:00.000Z';

    await vscode.workspace.fs.writeFile(
      canvasUri,
      Buffer.from(JSON.stringify(resumedCanvas, null, 2), 'utf8')
    );

    assert.ok(getCanvasTab(canvasUri), 'Expected custom editor to stay alive after externally appending a resume-generated blueprint output.');
    ...
    const reopened = await waitFor(async () => {
      ...
      if (current.nodes.find(node => node.id === 'bp-container')?.meta?.blueprint_last_run_status !== 'succeeded') { return null; }
    ...
    assert.equal(reopenedOutputs.length, 3, 'Expected resume-generated blueprint output to append as history after reopen.');
