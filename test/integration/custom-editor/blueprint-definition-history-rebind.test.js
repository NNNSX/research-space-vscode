...
async function buildTempCanvasUri() {
  ...
  const targetPath = path.join(tempDir, `blueprint-definition-history-rebind-${Date.now()}.rsws`);
...
async function writeBlueprintDefinition(filePath) {
  const definition = {
    ...
    metadata: {
      created_at: '2026-04-21T17:58:00.000Z',
...
async function run() {
  ...
  try {
    ...
    assert.ok(container, 'Expected definitions-rebind fixture canvas to contain a blueprint container.');
    assert.ok(boundOutput, 'Expected definitions-rebind fixture canvas to contain a blueprint bound output.');
    ...
    boundOutput.position = { x: 560, y: 250 };
    boundOutput.meta = {
      ...(boundOutput.meta ?? {}),
    ...
    rawCanvas.nodes.push({
      ...
      file_path: 'outputs/summary_definition_2.md',
      meta: {
        ai_provider: 'copilot',
        ai_model: 'gpt-4.1',
    ...
    rawCanvas.metadata.updated_at = '2026-04-21T18:00:00.000Z';
    ...
    const rebound = await waitFor(async () => {
      ...
      lastObservedState = {
        containerOutputSlots: current.nodes.find(node => node.id === 'bp-container')?.meta?.blueprint_output_slot_defs?.map(slot => slot.id) ?? [],
        placeholders: current.nodes
          .filter(node => node.meta?.blueprint_placeholder_kind === 'output')
          .map(node => ({ id: node.id, slotId: node.meta?.blueprint_placeholder_slot_id })),
        boundOutputs: getBoundOutputs(current).map(node => ({
          id: node.id,
          slotId: node.meta?.blueprint_bound_slot_id,
          slotTitle: node.meta?.blueprint_bound_slot_title,
          position: node.position,
        })),
        edges: current.edges
          .filter(edge => edge.edge_type === 'data_flow' || edge.edge_type === 'ai_generated')
          .map(edge => ({ source: edge.source, target: edge.target, edge_type: edge.edge_type, role: edge.role })),
      };
      ...
      if (currentContainer.meta.blueprint_output_slot_defs[0]?.id !== 'output_slot_summary') {
        return null;
      }
      if (outputs.some(node => node.meta?.blueprint_bound_slot_id !== 'output_slot_summary')) {
        return null;
      }
      ...
      return current;
    }, 6000, 150).catch(error => {
      throw new Error(`${error.message}\nObserved state: ${JSON.stringify(lastObservedState, null, 2)}`);
    });

    const reboundPlaceholder = findOutputPlaceholder(rebound, 'output_slot_summary');
    assert.ok(reboundPlaceholder);
    assert.deepEqual(getSlotBindingTargets(rebound, 'output_slot_summary', reboundPlaceholder.id), ['bound-output', 'bound-output-2']);
    ...
    assert.equal(reopenedOutputs.length, 2, 'Expected both blueprint output history nodes to survive definition-driven rebind + reopen.');
    assert.ok(reopenedPlaceholder);
    assert.ok(reopenedOutputs.every(node => node.meta?.blueprint_bound_slot_id === 'output_slot_summary'));
